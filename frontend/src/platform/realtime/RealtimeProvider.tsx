...
export function RealtimeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    hub.onreconnected(async () => { setStatus('connected'); await Promise.all([...subscriptions.current].map((id) => hub.invoke('SubscribeToHomeServicesBooking', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToChatConversation', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToSchoolMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToWarehouseMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToHomeServicesMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToTravelMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToRailwayMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToHotelMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToNewsMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToJobsMessage', id))) })
