...
export function RealtimeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    hub.onreconnected(async () => { setStatus('connected'); await Promise.all([...subscriptions.current].map((id) => hub.invoke('SubscribeToHomeServicesBooking', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToChatConversation', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToSchoolMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToWarehouseMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToHomeServicesMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToTravelMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToRailwayMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.Invoke('SubscribeToHotelMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToNewsMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToJobsMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToCommerceMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToBankMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToMedicalMessage', id))); await Promise.all([...chatSubscriptions.current].map((id) => hub.invoke('SubscribeToPresence', id))); await Promise.all([...chatSubscriptions.current].map(async (conversationId) => { try { await fetchMessageHistory(conversationId); } catch (e) { /* history fetch is best-effort */ } })) })
  ...
  const subscribeToTypingIndicator = async (conversationId: string) => {
    chatSubscriptions.current.add(conversationId)
    if (connection.current?.state === HubConnectionState.Connected) await connection.current.invoke('SubscribeToTypingIndicator', conversationId)
  }
  ...
  const fetchMessageHistory = async (conversationId: string) => {
    if (!connection.current?.state?.Equals(HubConnectionState.Connected)) return
    try {
      const response = await fetch(`${BASE}/api/v1/chat/conversations/${conversationId}/messages?limit=50`, {
        headers: { Authorization: `Bearer ${getToken()}` }
      })
      if (response.ok) {
        const data = await response.json()
        // TODO: populate chat state with returned messages
        // const messages = data.data?.messages || []
        // setChatMessages(conversationId, messages)
      }
    } catch (error) {
      // best-effort; don't break reconnect
    }
  }
