...
import type { Notification } from '../../services/school/types'
...
export default function InteriorNotifications() {
  const { items, add, update, remove } = useLocalCollection<Notification>('interior:notifications', NOTIFICATION_SEED)
  ...
  const columns: DataColumn<Notification>[] = [
    ...
    { key: 'read', header: 'Read', render: n.read ? <span className="text-emerald-600 text-sm font-medium">Read</span> : <span className="text-amber-600 text-sm font-medium">Unread</span> },
  ]
      ...
      <Card>
        ...
        <CardContent>
          <DataTable
            ...
            rowKey={(n) => n.id}
            pageSize={10}
            exportFilename="interior-notifications"
            emptyIcon={<Bell className="w-6 h-6" />}
            emptyTitle="No notifications"
            emptyDescription="Create notifications for the interior app."
            toolbar={
