...
const emptyForm = { name: '', client: '', startDate: '', budget: '0' }
...
export default function WarehouseProjects() {
  ...
  function openEdit(p: ProjectRecord) { setEditing(p); setForm({ name: p.name, client: p.client, startDate: p.startDate, budget: String(p.budget) }); setModalOpen(true) }

  function save() {
    if (!form.name.trim()) return
    const payload = { name: form.name.trim(), client: form.client.trim() || 'Internal', startDate: form.startDate || new Date().toISOString().slice(0, 10), budget: Number(form.budget) || 0 }
