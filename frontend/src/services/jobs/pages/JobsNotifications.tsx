...
export default function JobsNotifications() {
  const { items, add, update, remove } = useLocalCollection<Notification>('jobs:notifications', NOTIFICATION_SEED)
      ...
      <Card>
        ...
        <CardContent>
          <DataTable
            ...
            rowKey={(n) => n.id}
            pageSize={10}
            exportFilename="jobs-notifications"
            emptyIcon={<Bell className="w-6 h-6" />}
            emptyTitle="No notifications"
            emptyDescription="Create notifications for the jobs app."
            toolbar={
