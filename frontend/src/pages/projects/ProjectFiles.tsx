...
import { ArrowLeft, FolderPlus, FilePlus2, FolderOpen, FileText, Link2, Calendar } from 'lucide-react'
    ...
    <>
      <div className="page-head">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => { setFileName(''); setFilePath(''); setFolderName(''); setErr(''); setOpen('file') }}>
      ...
      <Card>
        {files.length === 0 ? (
          <Empty
            ...
            action={<Button onClick={() => { setFileName(''); setFilePath(''); setFolderName(''); setErr(''); setOpen('file') }}><FilePlus2 className="w-4 h-4" /> Add File</Button>}
          />
        ) : (
      ...
      {/* Add File modal */}
      <Modal open={open === 'file'} onClose={() => setOpen(null)} title="Add File" description="Attach a file reference to this project" size="md">
        <form onSubmit={(e) => { e.preventDefault(); addFile() }} className="space-y-5">
          <div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            <Button type="button" variant="outline" onClick={() => setOpen(null)}>Cancel</Button>
            <Button type="submit" disabled={saving}>{saving ? 'Saving…' : 'Add File'}</Button>
          </div>
        </form>
      </Modal>
    </>
