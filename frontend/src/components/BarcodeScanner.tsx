...
export default function BarcodeScanner({ open, onClose, onResult, title = 'Scan Barcode / QR' }: BarcodeScannerProps) {
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
      ...
      .catch(() => setCamError('Camera unavailable â€” enter the code manually below.'))
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} size="md">
      <div className="space-y-4">
        {supported === null ? (
          <div className="text-sm text-muted text-center py-8">Checking scanner supportâ€¦</div>
        ) : supported ? (
          camError ? (
            <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-sm text-amber-600 flex items-start gap-2">
              <IoScan className="w-4 h-4 mt-0.5 flex-shrink-0" />
              {camError}
            </div>
          ) : (
            <div className="relative rounded-xl overflow-hidden bg-black aspect-video">
              ...
              <div className="absolute bottom-2 left-2 text-white/80 text-xs flex items-center gap-1.5 bg-black/50 rounded-md px-2 py-1">
                <IoScan className="w-3.5 h-3.5" /> Point at a barcode or QR code
              </div>
            </div>
          )
        ) : (
          <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-sm text-amber-600 flex items-start gap-2">
            <IoBarcode className="w-4 h-4 mt-0.5 flex-shrink-0" />
        ...
        <div className="border-t border-border pt-4">
          ...
          <div className="flex gap-2 mt-1">
            <input
              ...
              placeholder="Type or paste a codeâ€¦"
              autoFocus
